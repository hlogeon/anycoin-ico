...
var AnythingAppTokenPreSale = artifacts.require("./AnythingAppTokenPreSale.sol");
...
module.exports = async function(deployer) {
  ...
  deployer.link(SafeMath, AnythingAppTokenPreSale);
  await deployer.deploy(AnythingAppToken).then(async function() {
    ...
    await deployer.deploy(AnythingAppTokenPreSale, 
      token,
      beneficiary,
    ...
    const preSaleInstance = await web3.eth.contract(AnythingAppTokenPreSale.abi).at(AnythingAppTokenPreSale.address);
    ...
    await ethProvider.setWatcher(AnythingAppTokenPreSale.address, { from: web3.eth.accounts[0] });
