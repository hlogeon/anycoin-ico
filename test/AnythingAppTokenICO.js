...
const AnythingAppTokenICO = artifacts.require("AnythingAppTokenICO");
...
contract('AnythingAppTokenICO', function (accounts) {
  ...
  it('should send tokens to purchaser (bonus 20%, 15%)', async function () {
    await this.whiteList.addInvestorToWhiteList(accounts[2]);
    
    // bonus 20%  
    await this.crowdsale.sendTransaction({value: tokensToAmount(417), from: accounts[2]});

    const balance1 = await this.token.balanceOf(accounts[2]);
    assert.equal(balance1.valueOf(), 500.4 * 10**18);

    const crowdsaleBalance1 = await this.token.balanceOf(this.crowdsale.address);
    assert.equal(crowdsaleBalance1.valueOf(), (totalTokens - 500.4) * 10 ** 18);

    const collected1 = await this.crowdsale.collected();
    assert.equal(collected1.valueOf(), tokensToAmount(417));
    ...
    assert.equal(tokensSold1.valueOf(), 500.4 * 10 ** 18);

    // bonus 15%
    await this.whiteList.addInvestorToWhiteList(accounts[3]);
    await this.crowdsale.sendTransaction({value: tokensToAmount(10), from: accounts[3]});

    const balance2 = await this.token.balanceOf(accounts[3]);
    assert.equal(balance2.valueOf(), 11.5 * 10**18);

    const crowdsaleBalance2 = await this.token.balanceOf(this.crowdsale.address);
    assert.equal(crowdsaleBalance2.valueOf(), (totalTokens - (500.4 + 11.5)) * 10 ** 18);

    const collected2 = await this.crowdsale.collected();
    assert.equal(collected2.valueOf(), tokensToAmount(10 + 417));
    ...
    assert.equal(tokensSold2.valueOf(), (11.5 + 500.4) * 10 ** 18);
  });
  ...
  it('should send 5% referral bonus', async function () {
    ...
    assert.equal(balanceOf2.valueOf(), 62.5 * 10 ** 18); // (50 + 20%) + (50 + 5%)
  ...
  it('should not add referral bonus to tokensSold if no referral of investor', async function () {
    ...
    const balanceOf2 = await this.token.balanceOf(accounts[2]);
    assert.equal(balanceOf2.valueOf(), 60 * 10 ** 18);
    ...
    assert.equal(tokensSold.toNumber(), 60 * 10 ** 18);
  });
});
