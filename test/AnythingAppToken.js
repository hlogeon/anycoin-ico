...
contract('AnythingAppToken', function(accounts) {
  it("should put 400000000 ANY to supply and in the first account", async function () {
    const instance = await AnythingAppToken.new();
    const balance = await instance.balanceOf(accounts[0]);
    const supply = await instance.totalSupply();

    assert.equal(balance.valueOf(), 400000000 * 10 ** 18, "First account (owner) balance must be 400000000");
    assert.equal(supply.valueOf(), 400000000 * 10 ** 18, "Supply must be 400000000");
  });
  ...
  it("should not allow to release by not release agent", async () => {
    let token = await AnythingAppToken.new();
  ...
  it("should allow transfer when token is released", async function() {
    ...
    assert.equal(balance0.valueOf(), 399999900 * 10 ** 18);
  ...
  it("should allow transfer when token is released - fractional value", async function() {
    ...
    const balance0 = await token.balanceOf(accounts[0]);
    assert.equal(balance0.valueOf(), 399999999999900000000000000);
  ...
  it("should allow to burn by owner", async function() {
    ...
    assert.equal(balance.valueOf(), 399000000 * 10 ** 18);

    const supply = await token.totalSupply().valueOf();
    assert.equal(supply.valueOf(), 399000000 * 10 ** 18);
  });
  ...
  it("should not allow to burn more than balance", async function() {
    ...
    try {
      await token.burn(400000001 * 10 ** 18);
    } catch (error) {
      return assertJump(error);
    }
    assert.fail('should have thrown before');
  ...
  it("should allow to burn from by owner", async function() {
    ...
    assert.equal(supply.valueOf(), 399500000 * 10 ** 18);
